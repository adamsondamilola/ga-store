    ...
    <footer className={`w-full bg-[#18261f] pb-8 pt-14 text-[#d7ddd4] ${className}`.trim()}>
      <div className="w-full max-w-none px-4 md:px-6 lg:px-8">
          ...
          <div className="grid grid-cols-1 gap-8 lg:grid-cols-[1.2fr_0.8fr_0.8fr_1fr]">
            <div>
              <div className="inline-flex items-center gap-3 rounded-full border border-white/10 bg-white/5 px-4 py-2">
                <img src={AppImages.logo} alt="TOWG" className="h-8 w-8 rounded-full object-cover" />
                <span className="text-sm font-semibold uppercase tracking-[0.26em] text-white">TOWG</span>
              </div>
              <h3 className="mt-5 text-2xl font-semibold text-white">Natural wellness, polished shopping, and trusted delivery.</h3>
              <p className="mt-4 max-w-md text-sm leading-7 text-[#b9c2b8]">
                TOWG Natural and Wellness brings together premium supplements and a calmer shopping experience designed to build trust from first click to checkout.
              </p>
            ...
            <div>
              ...
              <div className="space-y-4 text-sm text-[#b9c2b8]">
                <div className="flex items-start gap-3"><FiMail className="mt-1 text-white/70" /><span>{AppStrings.infoEmail}</span></div>
                <div className="flex items-start gap-3"><FiPhone className="mt-1 text-white/70" /><span>{AppStrings.phoneNumber}</span></div>
                <div className="flex items-start gap-3"><FiMapPin className="mt-1 text-white/70" /><span>{AppStrings.officeAddress}</span></div>
              </div>
            </div>
          </div>
        ...
        <div className="mt-8 text-center text-sm text-[#8f9a8f]">
          <p>&copy; {new Date().getFullYear()} {AppStrings.title} Natural and Wellness. All rights reserved.</p>
      ...
      <a href={AppStrings.whatsApp ? `https://wa.me/${AppStrings.whatsApp}` : "https://wa.me/2347052457688"} target="_blank" rel="noopener noreferrer" className="fixed bottom-6 right-6 z-40 group" aria-label="Chat on WhatsApp">
