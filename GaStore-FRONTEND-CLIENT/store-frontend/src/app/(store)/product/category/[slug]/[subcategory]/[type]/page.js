...
export async function generateMetadata({ params, searchParams }) {
  const { slug, subcategory, type } = params;

  return {
    title: formatSlugToTitle(type) || AppStrings.title,
...
export default async function ProductTypePage({ params, searchParams }) {
  ...
  const { slug, subcategory, type } = params;
  const formattedTitle = formatSlugToTitle(type);
  const searchTerm = slugToString(type) || search;
